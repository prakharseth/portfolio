import Head from 'next/head'
// import Image from 'next/image'
// import p from "..public/p.png";
// import styles from '../styles/Home.module.css'


export default function Home() {
  return (
    <>
      <Head>
        <link rel='icon' type='image/png' href='/p.png' />
      </Head>
      <div className='bg-[#1D1D1D] h-screen text-white flex flex-col justify-center px-5 '>
        {/* h1 */}
        <div>
          <div className='tag '>{"<h1>"}</div>
          {/* <Image src={p} /> */}
          <div className='me pl-5 '>
            Hi, I’m <span className='plogo '>P</span>rakhar, Web Developer
          </div>
          <div className='tag text-right pr-5'>{"</h1>"}</div>
        </div>

        {/* p1 */}
        <div>
          <div className='tag pt-5 pb-1'>{"<p>"}</div>
          <div className='font-open px-5 text-base text-[#808080] font-light'>
            Front-end Developer
          </div>
          <div className='tag pt-2'>{"</p>"}</div>
        </div>

        <button className='mt-8 tracking-widest w-36 border text-center border-[#05FDD8]'>
          <a href='mailto:sethprakhar12@gmail.com'>
            <div className='font-open text-sm text-[#05FDD8] px-4 py-1'>
              Contact me !
            </div>
          </a>
        </button>
      </div>
    </>
  );
}

// export default function Home() {
//   return (
//     <div className={styles.container}>
//       <Head>
//         <title>Create Next App</title>
//         <meta name='description' content='Generated by create next app' />
//         <link rel='icon' href='/favicon.ico' />
//       </Head>

//       <main className={styles.main}>
//         <h1 className='{styles.title} font-'>
//           Welcome to <a href='https://nextjs.org'>Next.js!</a>
//         </h1>

//         <p className={styles.description}>
//           Get started by editing{" "}
//           <code className={styles.code}>pages/index.js</code>
//         </p>

//         <div className={styles.grid}>
//           <a href='https://nextjs.org/docs' className={styles.card}>
//             <h2>Documentation &rarr;</h2>
//             <p>Find in-depth information about Next.js features and API.</p>
//           </a>

//           <a href='https://nextjs.org/learn' className={styles.card}>
//             <h2>Learn &rarr;</h2>
//             <p>Learn about Next.js in an interactive course with quizzes!</p>
//           </a>

//           <a
//             href='https://github.com/vercel/next.js/tree/canary/examples'
//             className={styles.card}
//           >
//             <h2>Examples &rarr;</h2>
//             <p>Discover and deploy boilerplate example Next.js projects.</p>
//           </a>

//           <a
//             href='https://vercel.com/new?utm_source=create-next-app&utm_medium=default-template&utm_campaign=create-next-app'
//             className={styles.card}
//           >
//             <h2>Deploy &rarr;</h2>
//             <p>
//               Instantly deploy your Next.js site to a public URL with Vercel.
//             </p>
//           </a>
//         </div>
//       </main>

//       <footer className={styles.footer}>
//         <a
//           href='https://vercel.com?utm_source=create-next-app&utm_medium=default-template&utm_campaign=create-next-app'
//           target='_blank'
//           rel='noopener noreferrer'
//         >
//           Powered by{" "}
//           <span className={styles.logo}>
//             <Image src='/vercel.svg' alt='Vercel Logo' width={72} height={16} />
//           </span>
//         </a>
//       </footer>
//     </div>
//   );
// }
